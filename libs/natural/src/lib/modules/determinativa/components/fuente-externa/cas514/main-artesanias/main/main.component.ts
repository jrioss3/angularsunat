...
import { Sfec514ArtesaniasMantenimientoComponent } from '../mantenimiento/mantenimiento.component';
...
export class Sfec514MainArtesaniasComponent extends RenderizarPaginacion implements OnInit, OnDestroy, AfterViewInit {
  ...
  public ArtesaniasTotalS: number;
  public Artesanias514: LCas514Detalle[];
  private listCompArtesanias;
  ...
  public montoLimiteDeduccion: number;
  ...
  constructor(
    ...
    private cus32Service: ParametriaFormulario,
  ...
  ngOnInit(): void {
    ...
    this.indice = [...this.lista514Cabecera].findIndex(x => x.indTipoGasto === this.tipoGastoArtesanias);
    this.obtenerListaYMontoTotalArtesanias();
    ...
    //console.log("UIT y porcentajededuccionlimite : " + this.UIT + " - " + this.porcentajeLimiteDeduccionUIT);
    this.montoLimiteDeduccion = this.UIT * (this.porcentajeLimiteDeduccionUIT / 100);
    //console.log("monto limite para todos los casos : " +  this.montoLimiteDeduccion);
  ...
  public ObtenerComprobante(val: string): string {
    const descripcionComp = this.listCompArtesanias.filter(x => x.val === val);
  ...
  public agregarOActualizar(id?: LCas514Detalle[], index?: number): void {
    const modal = {
      titulo: 'Artesanias y Turismo',
      accion: 'Artesanias'
    };
    const modalRef = this.modalService.open(Sfec514ArtesaniasMantenimientoComponent, this.funcionesGenerales.getModalOptions({}));
    modalRef.componentInstance.inputListaBienes = this.Artesanias514;
  ...
  public eliminarArtesanias(index: number): void {
  ...
  private calculoMontoyActualizarPredeclaracion(data: LCas514Detalle[], index: number): void {
    if (data !== null) { this.Artesanias514 = data; }
    if (index !== null) { this.Artesanias514.splice(index, 1); }
    this.ArtesaniasTotalS = this.Artesanias514.filter(x => x.indEstFormVirt === ConstantesCasilla514.regValido).
      reduce((total, Artesanias) => total + Number(Artesanias.mtoDeduccion), 0);
    this.lista514Cabecera[this.indice].casilla514Detalle.lisCas514 = this.Artesanias514;
    this.lista514Cabecera[this.indice].mtoGasto = this.ArtesaniasTotalS;
    SessionStorage.setPreDeclaracion(this.preDeclaracion);
    this.total.emit(this.ArtesaniasTotalS);
  }

  private calcularMontoDeducir(): void {
    this.Artesanias514.map(e => new CalcularMontoMaximoDeducir(e, this.anioEjercicio).getMontoDeduccionActualizadoArtesania());
    this.ArtesaniasTotalS = this.Artesanias514.filter(x => x.indEstFormVirt === ConstantesCasilla514.regValido).
      reduce((total, Artesanias) => total + Number(Artesanias.mtoDeduccion), 0);
  ...
  private obtenerListaYMontoTotalArtesanias(): void {
    this.lista514Cabecera.map(x => {
      switch (x.indTipoGasto) {
        case this.tipoGastoArtesanias: {
          this.Artesanias514 = x.casilla514Detalle.lisCas514;
          this.ArtesaniasTotalS = x.mtoGasto;
          break;
        }
      }
    });
  ...
  private obtenerListasParametria(): void {
    this.listCompArtesanias = this.cus32Service.obtenerTipoComprobante_cus32_Artesanias();
    //this.listCompArtesanias = this.listCompArtesanias.filter(x => x.val === ConstantesCasilla514.codBoleta);
    this.listaFormaPago = this.cus32Service.obtenerFormaPago();
  }
